...
class GrandKruDistController extends Controller
{
    //
    public function dist1(){
    	...
    	$pdf=PDF::loadView('kruDist1',['member'=>$member]);

        //converting the view(page) called members to a pdf file in the browser
    	return $pdf->stream('kruDist1.pdf');
    }
     ...
     public function dist2(){
    	...
    	//$member = Members::all();
         $member = DB::table('members')
            ...
            ->get();
    	//Displaying the view(page) called members
    	$pdf=PDF::loadView('kruDist2',['member'=>$member]);

        //converting the view(page) called members to a pdf file in the browser
    	return $pdf->stream('kruDist2.pdf');
    }
}
